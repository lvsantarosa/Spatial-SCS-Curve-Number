...
tribble(
  ...
  ,'https://drive.google.com/uc?export=download&id=1Qw8y_IjzK8l384oUU_PX7BB4a2i7_J31', 'Soil/sand.tif'
  ,'https://drive.google.com/uc?export=download&id=1YMzLPDt7weEbSg4x5mk28tiqqux8sdO2', 'Soil/silt.tif'
) %>%
  pwalk(function(url, destfile) {
    if (!file.exists(destfile)) {
      download.file(url = url, destfile = destfile)
    } else {
      glue('File {destfile} already exists! Skipping download...') %>% print()
    }
  })
